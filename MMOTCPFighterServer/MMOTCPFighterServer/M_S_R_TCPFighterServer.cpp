...
// 연결, 읽기, 쓰기 프로세스
void SelectProc(fd_set r_set, fd_set w_set, int beginIter, int endIter);
BOOL AcceptProc();
...
void NetworkIOProcess()
{
	...
	for (settingIter = playerList.begin(); settingIter != playerList.end(); ++settingIter)
	{
		...
		if (socketCount >= FD_SETSIZE)
		{
			...
			//endID = (*settingIter)->ID;
			SelectProc(r_set, w_set, beginID, endID);

			FD_ZERO(&r_set);
			FD_ZERO(&w_set);

			socketCount = 0;
			//beginID = (*settingIter)->ID;
		}
	}
