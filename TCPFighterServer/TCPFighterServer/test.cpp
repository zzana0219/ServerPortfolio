...
void Update()
{
	...
	//printf("Update elapsedTime : %f ms\n", elapsedTime * 1000);
	QueryPerformanceCounter(&startFrame);
...
void ReadProc(PLAYERINFO* player)
{
	...
	//char r_buffer[1000];
	int recvNum = recv(player->Socket, /*r_buffer, 1000*/player->recvQ.GetRearBufferPtr(), player->recvQ.GetFreeSize(), 0);
	...
	while (player->recvQ.GetUseSize() > 0)
	{
		...
		//if (recvHeader->byType == dfPACKET_CS_ATTACK3)
		//{
		CPacket testPacket;
		...
		PacketProc(player, recvHeader->byType, &testPacket);
			//short shX, shY;
			//BYTE byDirection;
			//testPacket >> byDirection >> shX >> shY;
			//
			//printf("%X, %d, %d\n",byDirection, shX, shY);
			//netPacketProc_Attack_3(player, &testPacket);
		//}
		//else
			//PacketProc(player, recvHeader->byType, packet);
			//PacketProc(player, recvHeader->byType, player->recvQ.GetFrontBufferPtr());
...
void WriteProc(PLAYERINFO* player)
{
	...
	int peekSize = player->sendQ.Peek(/*buffer*/player->sendQ.GetFrontBufferPtr(), useSize);
	if (peekSize != useSize)
	{
		printf("보내기 사이즈 안맞음\n");
		//delete[] buffer;
	...
	int sendNum = send(player->Socket, /*buffer*/player->sendQ.GetFrontBufferPtr(), peekSize, 0);
...
void Disconnect()
{
	...
	while (deleteIter != playerList.end())
	{
		...
		//mpDeleteClient(&header, &packet, (*deleteIter)->ID);
		//SendBroadCast(NULL, (char*)&header, sizeof(ST_HEADER), (char*)&packet, sizeof(DELETE_CLIENT));
...
bool netPacketProc_MoveStart(PlayerInfo* player, char* pPacket)
{
	...
	{
		//DisconnectClient(player);
		printf("MoveStart 아디 [%d] 서버 [%d] [%d] 클라 [%d] [%d]\n", player->ID, player->X, player->Y, m_packet->X, m_packet->Y);
		player->isConnect = false;
		G_Disconnect = true;
		return FALSE;
	}
...
bool netPacketProc_Attack_1(PlayerInfo* player, char* pPacket)
{
	...
	CPacket packet;
	mpMoveAttack1(&packet, player->ID, player->Direction, player->X, player->Y);
	SendBroadCast(player, &packet);
...
bool netPacketProc_Attack_2(PlayerInfo* player, char* pPacket)
{
	...
	CPacket packet;
	mpMoveAttack2(&packet, player->ID, player->Direction, player->X, player->Y);
	SendBroadCast(player, &packet);
...
bool netPacketProc_Attack_3(PlayerInfo* player, char* pPacket)
{
	...
	CPacket packet;
	mpMoveAttack3(&packet, player->ID, player->Direction, player->X, player->Y);
	SendBroadCast(player, &packet);
